...
#' If `k` is a vector of length > 1, only the first element will be used.
#' Specifying only one or two urns will respectively return `k` or call [stats::rhyper()] and issue a warning.
#'
#' If \eqn{k = \sum n} then the function should return `n`.
#'
#' @param n a vector describing the number of balls in each urn
#' @param k the total number of balls to draw from the urns
#'
#' @return A vector or array corresponding to the number of balls sampled from each of the urns, in the same format as `n`.
#'
#' @seealso [stats::rhyper()] for sampling two different colours of balls from a single urn (equivalent to sampling one colour from two urns)
#'
#' @examples
