...
predicted_citations <- fitted_citations('Bka', citations, clusters)
profile_resids <- profile_residuals(predicted_citations, citations[, 'Bka'])

library(tidyverse)
theme_set(theme_bw())
resids_df <- data_frame(journal = rownames(profile_resids),
                        residual = drop(profile_resids),
                        fitted = drop(predicted_citations),
...
ggqqline <- function(vec, distribution = qnorm, probs = c(.25, .75), ...) {
  ...
  y <- quantile(vec, probs)
  x <- distribution(probs, ...)
  slope <- diff(y) / diff(x)
  intercept <- y[1L] - slope * x[1L]
  return(geom_abline(intercept = intercept, slope = slope))
}
...
ggplot(resids_df) +
  ...
  geom_text(aes(colour = journal == 'Bka')) +
  scale_colour_manual(values = c('black', 'tomato2'), guide = 'none') +
  ggqqline(profile_resids) +
  labs(x = 'Normal quantiles',
       y = 'Sample quantiles',
       title = 'Profile residuals for \'Biometrika\'')
```

Using the `community_residuals()` function from the **scrooge** package, we can calculate the residual sum of squares of residuals of each journal based on the given community structure.
